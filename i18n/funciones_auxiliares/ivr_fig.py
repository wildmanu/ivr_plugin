...
def _tabla_info_bajo_ax(
    ...
    col_labels = ["Series", "Clave", "Municipio", "loc", "σ"]
    rows = []

    rows.append([
        "IVR GAU",
        "IIS-2025",
        "Muestra de 688 municipios",
        _fmt_num_ivr(loc_g, puntuacion),
        _fmt_num_ivr(scale_g, puntuacion)
    ])
    ...
    if loc_reg is not None:
        rows.append([
            "IVR LOC",
            any_cat,
            f"Muestra de {n_mun} municipios",
            _fmt_num_ivr(loc_reg, puntuacion),
            _fmt_num_ivr(scale_reg, puntuacion)
        ])

    if n_mun <= 1:
        rows.append([
            "IVR LOC",
            clave(cpro_med, cmun_med),
            fmt_mun(nmun_med),
            _fmt_num_ivr(loc_med, puntuacion),
            _fmt_num_ivr(scale_med, puntuacion)
        ])

    elif n_mun == 2:
        rows.append([
            "IVR LOC-MIN",
            clave(cpro_min, cmun_min),
            fmt_mun(nmun_min),
            _fmt_num_ivr(loc_min, puntuacion),
            _fmt_num_ivr(scale_min, puntuacion)
        ])

        rows.append([
            "IVR LOC-MAX",
            clave(cpro_max, cmun_max),
            fmt_mun(nmun_max),
            _fmt_num_ivr(loc_max, puntuacion),
            _fmt_num_ivr(scale_max, puntuacion)
        ])
    ...
    weights = [
        max(weights[j], minw_as_weight[j])
        for j in range(n_cols)
    ]
...
def visualizar_sigmoides_scipy(
        ...
        bins = 45, xmax_ate = int, fun_loc = False, # Cambiar a True
    ...
    for i, spec in enumerate(specs):
        ...
        #Logica para dos deciamles bien 
        pbi_txt = f"{pbi_out:.2f}"
        
        if puntuacion == ",":
            pbi_txt = pbi_txt.replace(".", ",")
        
        texto_atip = (
            f"Atípicos: {n_out}\n"
            f"Proporción: {pbi_txt}%"
        )
